...
- **Playwright-Projekte:** drei – `chromium` (alles außer den beiden Fake-Kamera-Specs, per `testIgnore`), `chromium-fakecam` und `chromium-fakecam-foreign` (je ein Spec, je ein eigenes Bild). Jedes Projekt trägt die **volle** Flag-Liste, weil `test.use({ launchOptions })` die Projekt-Argumente komplett ersetzt. Die `.y4m`-Dateien schreibt nur `tests/e2e/globalSetup.ts`, nie `playwright.config.ts` – Playwright löscht `test-results/` **vor** `globalSetup`.
